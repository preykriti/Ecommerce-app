    ...
    <div className="flex flex-col sm:flex-row mt-10 mx-2 gap-1 sm:gap-10">
      ...
      <div className="flex-1 sm:w-3/4">
        <div className="flex text-xs justify-between items-center gap-5">
          ...
          {/* sorting box */}
          <select className="border border-slate-400 h-8" onChange={(e)=>setSortType(e.target.value)}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 gap-y-6 mt-2 ">
           { filterProducts.map((item, index)=>(<ProductItem key={index} name={item.name} price={item.price} image={item.image} id={item._id} />))}
        </div>
      </div>
    </div>
  );
